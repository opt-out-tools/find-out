...
def test_contraction_unpacking_all():
    ...
    assert df_contractions['unpacked_values'].all() == df_contractions[
        'unpacked'].all()
...
def test_contraction_unpack_in_sentence():
    ...
    assert tweets.loc[
               3, 'contractions'] == "RT @baum_erik: Lol I am not surprised " \
...
def test_social_tokenizer():
    assert tokenizer(
        "@blanchettswhore Sunday,paul :)tweet? cele,blanchetts :) "
        "whore...?") == "@blanchettswhore Sunday , paul :) tweet ? cele , " \
                        "blanchetts :) whore . . . ?"
    assert tokenizer(
        "LIKEWISE 14:40@Reni__Rinse who's f****N dumb idea was it to change "
        "Thor to a girl?") == "LIKEWISE 14:40 @Reni__Rinse who ' s f****N " \
                              "dumb idea was it to change Thor to a girl ?"
    assert tokenizer(":-3;‑]O_o 3:‑) >.<") == ":-3 ;‑] O_o 3:‑) >.<"


def test_punctuation_cleaner_removes_colon():
    assert punctuation_cleaner(tokenized_tweets.loc[
                                   0, 'text']) == "RT @asredasmyhair " \
                                                  "Feminists take note " \
                                                  "#FemFreeFriday " \
...
def test_punctuation_cleaner_removes_exclamation_marks():
    tweet = "@MGTOWKnight @FactsVsOpinion . . . cue the NAFALT in 3 . . 2 . " \
            ". . 1 ! ! !"
    assert punctuation_cleaner(
        tweet) == "@MGTOWKnight @FactsVsOpinion cue the NAFALT in 3 2 1"


def test_lowercase():
    assert lowercase(tokenized_tweets.loc[
                         2, 'text']) == "@mgtowknight @factsvsopinion . . . " \
...
def test_normalize():
    ...
    assert tweets.loc[
               0, 'normalized'] == 'RT <user> : Feminists, take note. ' \
                                   '<hashtag> <hashtag> <url>'
...
def test_escape_sentence():
    assert escape_unicode(
        ['Iam\\xbf trying to \\xe4figure out \\xe1 unicode']) == [
